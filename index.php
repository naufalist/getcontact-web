...
try {
  switch ($_SERVER["REQUEST_METHOD"]) {
    case "POST":
      ...
      $final_key = trim(htmlspecialchars($_POST['final_key']));
      ...
      #region Validate final key

      if ($final_key == null || $final_key == "") {
        $alert["type"] = "warning";
        $alert["message"] = "Form data is invalid";
        $errors["final_key"] = "Final key is invalid";
        break;
      }
      ...
      if (!isset($source_type) || empty($source_type || is_null($source_type))) {
        $alert["type"] = "warning";
        $alert["message"] = "Form data is invalid";
      ...
      if ($source_type == "search") {
        $source_endpoint = GTC_API_EP_SEARCH;
      } elseif ($source_type == "profile") {
        $source_endpoint = GTC_API_EP_NUMBER_DETAIL;
      } else {
        $alert["type"] = "warning";
        $alert["message"] = "Form data is invalid";
        $errors["source_type"] = "Source type is invalid (2)";
        break;
      }

      #endregion

      #region Prepare for cURL
      ...
      $data = json_encode([
        ...
        "source" => $source_type, // profile | search
        "token" => $token,
      ]);
      ...
      curl_setopt_array($curl, [
        CURLOPT_URL => GTC_API_BASE_URL . $source_endpoint,
      ...
      $response = json_decode(getcontact_decrypt($decoded_response->data, $final_key), true);

      #endregion

      #region Validate http status code

      if (!isset($response["meta"]["httpStatusCode"])) {
        $alert["type"] = "danger";
        $alert["message"] = "Invalid http response";
        break;
      }

      if ($response["meta"]["httpStatusCode"] !== 200) {
        $alert["type"] = "danger";
        $alert["message"] = "Invalid http status code: " . $response["meta"]["httpStatusCode"];
        break;
      }
      ...
      if ($response == null) {
        $alert["type"] = "warning";
        $alert["message"] = "Invalid response";
        break;
      }
    ...
    default:
      $alert["type"] = "danger";
      $alert["message"] = "Invalid request method";
      break;
  }
} catch (\Exception $ex) {
...
<body class="user-select-none">
  ...
  <nav class="navbar navbar-expand-lg fixed-top bg-body-tertiary">
    <div class="container">
      <a href="#" class="navbar-brand nav-link">GetContact</a>
            ...
            <div class="accordion-item">
              ...
              <div id="collapseOne" class="accordion-collapse collapse show" aria-labelledby="headingOne" data-bs-parent="#accordionExample">
                <div class="accordion-body">
                  ...
                  <?php if (isset($result['result']['profile'])): ?>
                    <div class="text-center">
                      <img src="<?php echo $result['result']['profile']['profileImage'] ?? "" ?>" class="rounded" alt="Profile Image">
                    </div>
                    <dl class="row">
                      <dt class="col-sm-3">Display Name</dt>
                      <dd class="col-sm-9"><?php echo $result['result']['profile']['displayName'] ?? '-' ?></dd>

                      <dt class="col-sm-3">Name</dt>
                      <dd class="col-sm-9"><?php echo $result['result']['profile']['name'] ?? '-' ?></dd>

                      <dt class="col-sm-3">Surname</dt>
                      <dd class="col-sm-9"><?php echo $result['result']['profile']['surname'] ?? '-' ?></dd>

                      <dt class="col-sm-3">Phone Number</dt>
                      <dd class="col-sm-9"><?php echo $result['result']['profile']['phoneNumber'] ?? '-' ?></dd>

                      <dt class="col-sm-3">Display Number</dt>
                      <dd class="col-sm-9"><?php echo $result['result']['profile']['displayNumber'] ?? '-' ?></dd>

                      <dt class="col-sm-3">Tag Count</dt>
                      <dd class="col-sm-9"><?php echo $result['result']['profile']['tagCount'] ?? '-' ?></dd>

                      <dt class="col-sm-3">Email</dt>
                      <dd class="col-sm-9"><?php echo $result['result']['profile']['email'] ?? '-' ?></dd>

                    </dl>
                  <?php elseif (isset($result['result']['tags'])): ?>
                    ...
                    <table id="result-tags" class="table table-sm">
                      ...
                      <tbody>
                        ...
                        if (isset($result) && !is_null($result) && is_countable($result)) {
                          foreach ($response["result"]["tags"] as $tag_key => $tag_value) {
                            $tag_key += 1;
                            echo '<tr>
                                    <th scope="row">' . $tag_key . '</th>
                                    <td>' . $tag_value['tag'] . '</td>
                                    <td class="text-center"><h6 class="fw-bold">' . $tag_value['count'] . '</h6></td>
                                  </tr>';
                          }
                        } else {
                          echo '<tr>
    ...
    <div class="row justify-content-center">
      <div class="col-12 col-md-8">
        <div class="card border-0">
          <div class="card-body">
            <form id="form" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) ?>" method="POST" onsubmit="return submitForm()">
              ...
              <!-- Final Key & Token Select -->
              <div class="row mb-3">
                <label for="final-key-token" class="col-sm-3 col-form-label">Final Key & Token</label>
                <div class="col-sm-9">
                  <select id="final-key-token" name="final_key_token" class="form-select" aria-label="Final Key & Token" disabled>
                    <option value="" selected disabled>-- Choose Final Key & Token --</option>
                  </select>
                  <div class="invalid-feedback"></div>
                  <div class="form-check mt-2">
                    <input class="form-check-input" type="checkbox" id="use-different-credentials">
                    <label class="form-check-label" for="use-different-credentials">
                      Use different credentials?
                    </label>
                    <span id="display-expired-at" class="badge bg-soft-primary" data-bs-theme="light">Expired at: -</span>
                  </div>
                </div>
              </div>
              ...
              <!-- Final Key Input -->
              <div class="row mb-3">
                <label for="final-key" class="col-sm-3 col-form-label">Final Key <small class="text-danger">*</small></label>
                <div class="col-sm-9">
                  <input type="text" class="form-control" id="final-key" name="final_key" placeholder="" value="<?php echo isset($old_values["final_key"]) ? $old_values["final_key"] : "3a2adf118bb013b99f492c8419592cd7940cdb344320e02aa74a1b877094886a"; ?>" autocomplete="off" readonly>
                  <div class="invalid-feedback">
                    <?php echo isset($errors["final_key"]) && !empty($errors["final_key"]) ? $errors["final_key"] : ""; ?>
                  </div>
                </div>
              </div>
              ...
              <!-- Token Input -->
              <div class="row">
                <label for="token" class="col-sm-3 col-form-label">Token <small class="text-danger">*</small></label>
                <div class="col-sm-9">
                  <input type="text" class="form-control" id="token" name="token" placeholder="" value="<?php echo isset($old_values["token"]) ? $old_values["token"] : "bxuIUB07327befeadca081fdc2d97d39dd06734a623dc37a172572371f"; ?>" autocomplete="off" readonly>
                  <div class="invalid-feedback">
                    <?php echo isset($errors["token"]) && !empty($errors["token"]) ? $errors["token"] : ""; ?>
                  </div>
                </div>
              </div>
              ...
              <div class="row mb-3">
                <div class="col-sm-9 offset-sm-3">
                  <a href="#" class="form-text text-primary text-decoration-underline" data-bs-toggle="modal" data-bs-target="#how-to-get-finalkey-and-token-modal">How to get Final Key and Token?</a>
                </div>
              </div>
              ...
              <!-- Submit Button -->
              <div class="d-grid gap-2 offset-sm-3 mt-4">
                <button id="submit-btn" type="button" onclick="submitForm()" class="btn btn-primary btn-md py-2" disabled>Submit</button>
              </div>
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h1 class="modal-title fs-5">How to get Final Key & Token</h1>
  ...
  <footer class="footer bg-body-tertiary" data-bs-theme="dark">
    <div class="container">
      <span class="text-muted">Made with <span style="color: #e25555;">&#10084;&#65039;</span> <a href="https://tools.naufalist.com/" class="text-decoration-none" target="_blank" title="Naufalist">naufalist</a></span>
    </div>
  </footer>
